...
| `distances`  | `tbl_df`     | `unique_id`     | Rice field distance calculation results |
...
The `wetdry` layer simply provides polygons outlining "wet" (river/floodway-exposed) and "dry" (levee-protected) areas of the Sacramento Valley.

```
wetdry
```
```{r dataset-wetdry, echo=FALSE}
wetdry |> head(5) |> knitr::kable()
```
