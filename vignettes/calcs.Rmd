...
The `calc_inv_mass_ts` function returns the growth in biomass over time for a selected number of days. This returns a simple `tibble` in the following format...

```
fields |> calc_inv_mass_ts(14) 
```
```{r calc-field-ts, echo=TRUE, message=FALSE}
fields |> calc_inv_mass_ts(14) |> head(10) |> knitr::kable()
```
...which can be pivoted as so if needed...
```
fields |> calc_inv_mass_ts(7) |> pivot_wider(names_from = day, values_from = total_prod_kg, names_prefix = "day_")
```
```{r calc-field-ts-pivot, echo=FALSE, message=FALSE, warning=FALSE}
fields |> calc_inv_mass_ts(7) |> 
