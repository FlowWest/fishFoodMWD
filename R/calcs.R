...
#' @param df An `sf` SpatialPolygonsDataFrame, such as the provided dataframe `fields`, which includes an attribute area_ac containing the field area in acres. Also works with an ordinary data.frame or tibble.
...
calc_inv_mass <- function(df, day) {
  df |>
    dplyr::mutate(area_m2 = area_ac / 4047,
